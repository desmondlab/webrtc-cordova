...
'Use Strict';
angular.module('App').controller('contactUsController', function($scope, $state, $localStorage, Popup, $cordovaCamera, Utils) {
...
$scope.add = function(product) {
    ...
    // if (angular.isDefined($scope.product)) { //Check if productForm is filled up.
      ...
      //uploadtest.forEach(function(account) {  
           ...
           console.log('step3');
            firebase.database().ref('enquiryList').push({
                name: $scope.product.name,
                contactEmail: $scope.product.email,
                contactNo: $scope.product.phone,

...
angular.module('App').controller('settingController', function($scope, $state, $localStorage, Popup, $cordovaCamera, Utils) {
  ...
  $scope.$on('$ionicView.enter', function() {
    ...
    firebase.database().ref().child('userProfile/'+$localStorage.userId).update({
    ...
    firebase.database().ref().child('loginEmail/'+$localStorage.userId).update({
                email: firebase.auth().currentUser.email,
                userId: $localStorage.userId,
              }).then(function(response) {
                  console.log('save email to firebase database');
...
// get user info
firebase.database().ref('userProfile').orderByChild('userId').equalTo(userId).once('value').then(function(userdetailtest) {
          ...
          if (userdetailtest.exists()) {
            ...
            // var userkey = userdetailtest.val();
             $scope.product.name = userdetailtest.child(userId).child('fullName').val();
             $scope.product.email = userdetailtest.child(userId).child('email').val();
             $scope.product.phone = userdetailtest.child(userId).child('phoneNo').val();
...
$scope.add = function(product) {
    ...
    if ($scope.isAdding) { 
           console.log('step3');
                ...
                firebase.database().ref().child('userProfile/'+userId).update({
                fullName: $scope.product.name,
                email: $scope.product.email,
                phoneNo: $scope.product.phone,
                userId: userId,
                UpdateDate: Date()
              }).then(function(response) {
                  console.log('step4');
